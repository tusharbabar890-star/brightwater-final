...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-stonewarm-200 bg-stonewarm-50/95 backdrop-blur">
      <div className="container-luxe flex min-h-20 items-center justify-between gap-6">
        <Link href="/" aria-label={`${site.name} home`} className="group">
          ...
          <span className="mt-1 block text-[11px] font-semibold uppercase tracking-[0.22em] text-stonewarm-700">
            Family Law
          </span>
        </Link>
        ...
        <button
          type="button"
          className="inline-flex h-11 w-11 items-center justify-center border border-stonewarm-300 text-navy-950 lg:hidden"
      ...
      {open ? (
        <div className="border-t border-stonewarm-200 bg-stonewarm-50 lg:hidden">
          <nav
            aria-label="Mobile navigation"
            className="container-luxe grid gap-1 py-5"
          >
            {allLinks.map((item) => (
              <Link
                key={item.href}
                ...
                className="border-b border-stonewarm-200 py-4 text-base font-semibold text-navy-950"
              >
                {item.label}
              </Link>
            ))}
